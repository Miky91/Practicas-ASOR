...
char* permisosls (char *ruta) {
    struct stat state;
    char permisos[4] = "";
    if ((lstat (ruta, &state))!=0) {
        perror ("Error al consultar el fichero");
        return 1;
    }
    
    //setuid, setgid, sticky
    if ((S_ISUID & state.st_mode)!=0)
        permisos[0]= 'S';
    
    if ((S_ISGID & state.st_mode)!=0)
        permisos[0]= 'S';
    
    if ((S_ISVTX & state.st_mode)!=0)
        permisos[0]= 'T';	
            
    //usuario
    switch ((S_IRWXU & state.st_mode)>>6) {
        case 0:
            strcat(permisos,"---");
            break;
    ...
    //grupo
    switch ((S_IRWXG & state.st_mode)>>3) {
        case 0:
            strcat(permisos,"---");
            break;
	...
	while(mensaje[0] != 'q'){
		...
		if (tammsg != -1){
			...
			if(mensaje[0] == 't'){
				...
				char ruta[256] = "";
				if (mensaje[2] == ' ' && mensaje[3] == '-' && mensaje[4] == 'l')
					sscanf(mensaje,"ls -l %s", ruta);
				else
					...
					do{
						...
						if(actual != NULL){
							...
							if(S_ISREG(fichero.st_mode)){
								...
								//showstring(nombres,strlen(nombres));
								if (mensaje[2] == ' ' && mensaje[3] == '-' && mensaje[4] == 'l')							
									strcat(nombres,permisosls(abspath));
								strcat(nombres, "\n");
							}
